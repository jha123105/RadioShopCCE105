...
public class InventoryPanel extends JPanel {
    ...
    private void loadInventoryData() {
        ...
        List<Radio> radios = DatabaseManager.getAllRadios(); // In-order traversal

        for (Radio radio : radios) {
            Object[] row = {
                    radio.getId(), radio.getModel(), radio.getBrand(),
                    radio.getType().getDisplayName(), String.format("$%.2f", radio.getPrice()),
                    radio.getQuantity(), radio.getFrequency(),
                    radio.getPowerOutput() + "W", radio.getDescription()
            };
    ...
    private void searchInventory() {
        ...
        // Optimized: Use AVL Tree search O(log n) if searching by exact name
        // or in-order traversal with filter O(n) for partial/type search
        List<Radio> radios;
        if (!searchTerm.isEmpty()) {
            radios = DatabaseManager.searchRadios(searchTerm);
        } else {
            radios = DatabaseManager.getAllRadios();
        }
    ...
    private void editRadio() {
        ...
        Radio radioToEdit = DatabaseManager.getRadioById(radioId); // O(log n) AVL Tree lookup
    ...
    private void deleteRadio() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            int radioId = (int) tableModel.getValueAt(selectedRow, 0);
            DatabaseManager.deleteRadio(radioId); // O(log n) AVL Tree delete
            loadInventoryData();
    ...
    private JDialog createRadioDialog(Radio radio) {
        JDialog dialog = new JDialog(
                ...
                radio == null ? "Add New Radio" : "Edit Radio", true);
        ...
        // If editing existing radio, pre-fill fields
        if (radio != null) {
        ...
        saveButton.addActionListener(e -> {
            try {
                ...
                if (model.isEmpty() || brand.isEmpty() || frequency.isEmpty()) {
                    JOptionPane.showMessageDialog(dialog, "Please fill in all required fields.");
                    return;
                }

                if (radio == null) {
                    // Add new radio - O(log n) AVL Tree insert
                    int newId = DatabaseManager.getNextRadioId();
                    Radio newRadio = new Radio(newId, model, brand, type, price, quantity,
                            frequency, power, description);
                    DatabaseManager.saveRadio(newRadio);
                } else {
                    // Update existing radio - O(log n) AVL Tree update
                ...
                loadInventoryData();
                JOptionPane.showMessageDialog(this, "Radio saved successfully!");
                dialog.dispose();

            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(dialog,
                        "Please enter valid numbers for Price, Quantity, and Power Output.");
            }
        });
